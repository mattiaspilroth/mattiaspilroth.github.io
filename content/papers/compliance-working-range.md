...
State the outcome that must not occur. Identify the pathways by which a compromise reaches it. Establish that what those pathways can achieve is limited by something the compromise cannot also defeat. Test the bound, record the result, and repeat when the plant changes.

A limit can be physical: a vessel rated above anything the process can generate, a valve that cannot pass more than the downstream will take, a line sized so the volume is not available. Nothing needs to intervene, because the outcome is not among the things the plant can do. Or the limit can be something that intervenes, and then it has to be beyond the reach of the authority assumed compromised. Being a different product, a different network or a different supplier does not put it there.
