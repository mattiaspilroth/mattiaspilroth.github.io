...
The priority order below is derived from the consequence profile of high-hazard continuous process environments operating under IEC 61511 or equivalent functional safety regulation. Different operational contexts with different consequence profiles require a different tier derivation. The framework's logic (sequencing by consequence and dependency, barrier dimensioning, explicit stopping point) transfers. The specific tier order does not. Section 11 carries the full applicability argument.
...
3. **Attack surface assessment:** Because step 1 is complete, all remaining paths crossing the isolated boundary are defined. Each connection crossing the zone boundary is assessed only for its capacity to affect the assessed zone. The same connection assessed from the adjacent zone evaluates impact in the opposite direction. Neither assessment is responsible for the other zone's exposure. This prevents scope expansion during assessment and ensures each zone's protection is evaluated against its own consequence profile. Scope is determined by technical capability to affect the assessed zone, not by data flow direction or policy configuration. A connection is in scope if it carries or can trigger modification capacity within the zone through any chain of automated action. Human-mediated action is the scope boundary on the outbound side: a connection carrying data that a human reads and then acts upon is outside the assessed zone's attack surface, as the write action belongs to the zone the human operates from. Each in-scope path is documented with business justification, permitted protocols, and authentication requirements. Paths without current business justification are removed, not documented.
...
5. **Residual risk acceptance:** Formally signed by a named risk acceptor who understands what they are accepting. The residual risk register distinguishes two control states: **Technically Blocked** (enforced by architectural condition that cannot be defeated by a single authorized action within the system's normal operating interfaces; requires physical modification to reverse) and **Procedurally Controlled** (dependent on configuration, policy, or human compliance; includes firewall rules, access control lists, and administrative procedures regardless of their technical implementation). The test for Technically Blocked is whether a single person with legitimate access can reverse the control without physical modification. If yes, the control is Procedurally Controlled. Every Procedurally Controlled item is a residual by definition, requires a named acceptor, and must be explicitly acknowledged as dependent on continued maintenance and human compliance under operational pressure.
...
CVE exploitation requires network reachability. A genuinely isolated OT asset with no reachable attack surface has near-zero security exposure from unpatched vulnerabilities. Patching in OT is not a maintenance activity with predictable outcomes. It is a change event carrying its own operational risk: firmware updates may alter validated behavior, reboots may not complete cleanly on aged hardware, and dependency chains in long-lifecycle systems are poorly documented. A stable, running system in a verified isolated environment carries less operational risk than the same system after an unnecessary patch cycle. Patching is a risk-based activity executed only where the tier risk assessment identifies a specific, reachable vulnerability whose exploitation risk exceeds the operational risk of the patch action itself. Applying IT patch cadence to isolated OT assets consumes scarce change windows, introduces unnecessary operational risk, and does not improve security posture. It is a framework import error.
